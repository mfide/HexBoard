...
enum class EVertextColor {vtWHITE, vtBLUE, vtRED};
...
class CVertex
{
	...
	public:
		const TVertexID	ID(void) const { return (vertex_id); }
		EVertextColor	Color;

		const TVertexEdgeList& EdgeList(void) const { return (edge_list); }

		void AddEdge(const TEdgeID edge_id);
		void RebuildEdgeList(void);

		// constructor
		CVertex(const CGraph& g, const TVertexID id) : Owner(g), vertex_id(id) { }
		CVertex(const CVertex& v) : Owner(v.Owner), vertex_id(v.ID()), Color(v.Color) { }

		bool operator==(const CVertex& x) { return((vertex_id == x.vertex_id) && (&Owner == &x.Owner)); }
		bool operator<(const CVertex& x) { return((vertex_id < x.vertex_id) && (&Owner == &x.Owner)); }
		bool operator>(const CVertex& x) { return((vertex_id > x.vertex_id) && (&Owner == &x.Owner)); }
};
...
class CEdge
{
	...
	public:
		...
		// constructor
		CEdge(const CGraph&	g, const CVertex& source, const CVertex& dest, const TEdgeID id = -1, const float val = 0.0f): 
			Owner(g), 
