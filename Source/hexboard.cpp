...
// destructor
CHexBoard::~CHexBoard(void)
{
	cout << "\nBYE !.. \n";
}

void CHexBoard::CreateHexBoardVertices(void)
{
...
void CHexBoard::CreateEdgesBetweenVertices(void)
{
...
void CHexBoard::CreateWinnerVerticesAndEdges(void)
{
	...
	{
		// from left to right
		graph.AddEdge(graph.GetVertex(left_vertex), graph.GetVertex(i*board_width_height), 1.0f);
		graph.AddEdge(graph.GetVertex(right_vertex), graph.GetVertex((i+1)*board_width_height - 1), 1.0f);

		// from top to bottom
		graph.AddEdge(graph.GetVertex(top_vertex), graph.GetVertex(i), 1.0f);
		graph.AddEdge(graph.GetVertex(bottom_vertex), graph.GetVertex(i + (board_width_height - 1)*board_width_height), 1.0f);
	}
}
...
void CHexBoard::OccupyVertex(const TVertexID v_id, const EVertextColor c)
{
	...
	unoccupied_vertices.erase(find(unoccupied_vertices.cbegin(), unoccupied_vertices.cend(), v_id));
}

// print hex board on the screen
void CHexBoard::PrintBoard(void)
{
	string row_spacer = " ";
	...
	for (int32_t x = 0; x < board_width_height; x++)
	{
		...
		{
			yplayer += static_cast<char>(toupper(VertexColorToStr(EVertextColor::vtBLUE)));
			yplayer += "   ";
		}
	}	
	cout << yplayer << "\n";
	...
	for (int32_t y = 0; y < board_width_height; y++)
	{
		...
		for (int32_t x = 0; x < board_width_height; x++)
		{
			...
			if (find(shortest_path.ShortestPath.cbegin(), shortest_path.ShortestPath.cend(), &v) == shortest_path.ShortestPath.cend())
			{
				cout << v.Color;
			}
			else
		...
		{
			cout << row_spacer << static_cast<char>(toupper(VertexColorToStr(EVertextColor::vtRED)));
			for (int32_t x = 0; x < board_width_height; x++)
			{
				cout << "    ";
			}
			cout << "        " << static_cast<char>(toupper(VertexColorToStr(EVertextColor::vtRED)));
			cout << "\n";
		}
	}
	// --------------------------------------------------------------------
...
void CHexBoard::ChoosePlayer(void)
{
...
// converts vertex id to coordinates so that user can understand the position which AI plays
...
float CHexBoard::DoMonteCarlo(int32_t id_inx, int32_t sim_count)
{
	...
	do
	{
		...
		while (!uovl.empty())
		{
			...
			// removing the end is faster and still random because of shuffle above
...
// against another program) move.When it is the “AI”’s turn, it is to evaluate 
// all legal available next moves and select a “best” move.Each legal move will 
// be evaluated using  ~1000 or more trials.Each trial winds the game forward 
// by randomly selecting successive moves until there is a winner.The trial is 
// counted as a win or loss.The ratio : wins / trials are the AI’s metric for 
// picking which next move to make.
...
void CHexBoard::NextPlayer(void)
{
...
{	
	...
	if (active_player == human_player)
	{
		...
		{
			string str;
			cout << "Human Player " << static_cast<char>(toupper(VertexColorToStr(active_player))) << " move: ";
			cin >> str;
			if (UserInputToVertextID(str, id))
			{
				// check whether selected place is empty
				valid_input = (graph.GetVertex(id).Color == EVertextColor::vtWHITE);
				if (!valid_input)
				{
	...
	else
	{
		...
		id = AI_MOVE(5000);
		cout << "AI Player " << static_cast<char>(toupper(VertexColorToStr(active_player))) << " move: " << VertextIDToCoordStr(id) << endl;
	}
...
// any winner?
bool CHexBoard::CheckForWinner(void)
{
...
void CHexBoard::Start(void)
{
	...
	do
	{
		...
		// before human player action, print the board so that he can see the latest status before his action
		if (active_player == human_player)
		{
			PrintBoard();
		}
	...
	} while (!CheckForWinner()); // check whether there is a winner after last move

	PrintBoard();
	cout << "Player " << static_cast<char>(toupper(VertexColorToStr(active_player))) << " won the game!" << endl;
}
