...
///////////////////////////////////////////////////////////////////////////
void CVertex::AddEdge(const TEdgeID id)
{
	edge_list.insert(id);
}

void CVertex::RebuildEdgeList(void)
{
	edge_list.clear();

	for (TEdges::const_iterator edge = Owner.EdgeList().cbegin(); edge != Owner.EdgeList().cend(); edge++)
	{
		if ((edge->From().ID() == vertex_id) || (edge->To().ID() == vertex_id))
		{
			AddEdge(edge->ID());
		}
	}
...
{
	return((&x.Owner == &Owner) && (x.From().ID() == from_vertex.ID()) && (x.To().ID() == to_vertex.ID()));
}
...
CGraph::CGraph(const uint32_t vertex_count, const float pertange_of_edge_density)
{
	// take seed from system timer in order to generate random numbers	
	default_random_engine generator(static_cast<uint32_t>(system_clock::now().time_since_epoch().count()));
	...
	for (uint32_t source = 0; source < vertex_count - 1; source++)
	{
		...
		for (uint32_t dest = source + 1; dest < vertex_count; dest++)
		{
			...
			if (density_distribution(generator) <= pertange_of_edge_density)
			{
				...
				AddEdge(GetVertex(source), GetVertex(dest), weight_distribution(generator));
			}
		}
	}
	...
	if (txt_stream_file.is_open())
	{
		...
		while (getline(txt_stream_file, line))
		{
			...
			else
			{
				...
				float edge_cost;
				string_stream >> from_vertex_index >> to_vertex_index >> edge_cost;

				AddEdge(GetVertex(from_vertex_index), GetVertex(to_vertex_index), edge_cost);
			}
		}
