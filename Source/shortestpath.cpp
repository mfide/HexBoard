...
// this is kind of linked list to store internal data required by shortest path algorithms
...
class CMinHeapPairComparator
{
	public:
		// compares distances
		bool operator() (const TMinHeapPair& x, const TMinHeapPair& y) { return(x.second > y.second); }
};
typedef priority_queue<TMinHeapPair, vector<TMinHeapPair>, CMinHeapPairComparator> TMinHeap;

// Dijkstra's_algorithm based on Pseudo code at wiki-pedia:
// http://en.wikipedia.org/wiki/Dijkstra's_algorithm
// 
bool CShortestPath::DijkstraShortestPath(const TVertexID from_index, const TVertexID to_index)
{
	TargetReached = false;
	ShortestPath.clear();
	TotalDistance = 0.0f;
	...
	if (graph.GetVertex(from_index).Color == graph.GetVertex(to_index).Color)
	{
		...
		dijkstra_data[from_index].dist = 0.0f;	// Distance from source to source
		Q.push(make_pair(from_index, dijkstra_data[from_index].dist));	// Start off with just the source node
		...
		while (!Q.empty() && !TargetReached)
		{
			...
			if (u_index == to_index)
			{
				...
				ShortestPath.push_front(&graph.GetVertex(u_index)); // add target first
				while (u->previous >= 0)
				{
			...
			else
			{
				for (const TEdgeID v : graph.GetVertex(u_index).EdgeList())
				{
					...
					if (graph.GetVertex(v_inx).Color == graph.GetVertex(from_index).Color)
					{
						// accumulate shortest dist from source
						float alt = u->dist + graph.GetEdge(v).Value();

						if ((alt < dijkstra_data[v_inx].dist) && !dijkstra_data[v_inx].visited)
						{
...
public:
	// compares distances
	bool operator() (const CEdge* x, const CEdge* y) { return(x->Value() > y->Value()); }
};

// Kruskal minimal spanning tree algorithm based on Pseudo code at wiki-pedia:
// http://en.wikipedia.org/wiki/Kruskal's_algorithm
// 
void CShortestPath::KruskalMinimalSpanningTree(void)
{
	MinimalSpanningTree.clear();
	MinimalSpanningTreeDistance = 0.0f;

	priority_queue<const CEdge*, vector<const CEdge*>, CMinEdgePointerComparator> Q;
